...
images: [1.PNG, 2.PNG]
excerpt: "How to make a timer that runs in the background on both Android and IOS."
featuredPost: false
keywords:
  [
    "100DaysOfCode",
...
We can now add our two and only state variables to the top of our functional App component. 3601 corresponds to 01 hour 00 mins 01 sec. timerOn will be toggled on or off by the start/stop button.
...
Now is a good time to add an onPress event listener to the button.
...
startTimer, when called, starts the countdown. The runBackgroundTimer method works on both iOS and Android. This will be called every 1s (1000ms). If there are any seconds left, then we will minus 1. When seconds hits 0, we return 0 to ensure that we don’t go into negative time.
...
Here is an example of how this works. We have 3601 seconds left.
